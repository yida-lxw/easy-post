...
'''
@Project  easy-post
@File     postimages_service.py
@IDE      PyCharm
@Author   yida
@Email    949226420@qq.com
@Date     2024.12.16 16:28
@Desc     PostImages图床API请求封装
'''
...
class PostImagesService:
    ...
    def login(self) -> dict:
        ...
        login_post_response = requests.post(PostImageRoute.login_page, headers=PostImagesHeader.login_post, data=data,
                                            allow_redirects=False)
        self.cookies = login_post_response.cookies.get_dict()
        valid_token_response = requests.get(login_post_response.headers['Location'], cookies=self.cookies,
                                            headers=PostImagesHeader.token_validator, allow_redirects=False)
        self.cookies = valid_token_response.cookies.get_dict()
        return self.cookies

    def get_galleries(self):
        galleries_response = requests.get(PostImageRoute.galleries, cookies=self.cookies,
                                          headers=PostImagesHeader.galleries)
    ...
    def set_gallery_upload_token(self, working_gallery: dict) -> None:
        gallery_upload_token_response = requests.get(f"{PostImageRoute.gallery_token}/{working_gallery.get('hex')}",
                                                     cookies=self.cookies, headers=PostImagesHeader.galleries)
    ...
    def _get_image_urls(self, url: str) -> dict:
        ...
        links = {'_'.join(el.find('input').get('id').split('_')[1:]): el.find('input').get('value') for el in
                 soup.find_all('div', {'class': 'input-group'})}
        return links
    ...
    def upload_image(self, image_path: str, optimize: bool = False) -> dict:
        ...
        upload_response = requests.post(PostImageRoute.upload_image, cookies=self.cookies,
                                        headers=PostImagesHeader.upload_image, data=data)
